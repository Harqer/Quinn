...
export async function decodeAudioData(
  ...
  // Safely construct Int16Array taking alignment and offset into account
  let dataInt16: Int16Array;
  if (data.byteOffset % 2 === 0 && data.byteLength % 2 === 0) {
    dataInt16 = new Int16Array(data.buffer, data.byteOffset, numSamples);
  } else {
    const alignedBuffer = new ArrayBuffer(numSamples * 2);
    const alignedUint8 = new Uint8Array(alignedBuffer);
    alignedUint8.set(data.subarray(0, numSamples * 2));
