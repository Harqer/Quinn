...
export const MainDashboard: React.FC = () => {
  ...
  return (
    <div className="flex h-full bg-surface text-on-surface relative">
      ...
      <div className="w-96 flex flex-col gap-4 p-4 border-l border-outline/10 bg-surface-container-low/30 backdrop-blur-xl">
          ...
          <div className="flex gap-4 items-center">
            <button
              ...
              className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${
                voiceStatus === 'listening' ? 'bg-error text-on-error shadow-lg shadow-error/40 scale-110' : 'bg-primary text-on-primary hover:brightness-110'
              }`}
            >
            ...
            <div className="flex-1 overflow-hidden">
              {messages.length > 0 ? (
                <p className="text-sm text-primary font-bold animate-pulse truncate">{messages[0]}</p>
              ) : (
                <div className="text-on-surface-variant text-sm italic">"Quinn, play something cinematic..."</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col p-4 rounded-2xl bg-surface-container border border-outline/5 space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] uppercase tracking-widest font-black px-2 py-1 rounded bg-surface-container-high text-on-surface-variant">
              Grid Play
            </span>
          </div>
