...
export class SpotifyService {
  ...
  async savePodcastToPlaylist(uid: string, trackUri: string): Promise<boolean> {
    ...
    try {
      ...
      if (!playlistId) {
        // Find or Create "Musically Podcasts" playlist
        ...
        const existing = playlistsData.items.find((p: any) => p.name === "Musically Podcasts");
        ...
        if (existing) {
          ...
          const createRes = await fetch(`https://api.spotify.com/v1/users/${userSpotifyId}/playlists`, {
            ...
            body: JSON.stringify({
              name: "Musically Podcasts",
              description: "Your AI-powered narratives generated by Musically and Quinn.",
              public: false
            })
          });
