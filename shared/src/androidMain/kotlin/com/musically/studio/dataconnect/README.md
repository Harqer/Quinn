...
with service ID `musically-studio`
...
For example, the "GetAlbums" query has no variables
and can be executed via the
[DefaultConnector.getAlbums]
property as follows:

```kotlin
val connector = DefaultConnector.instance
val queryResult = connector.getAlbums.execute()
println("GetAlbums query returned: ${queryResult.data}")
```
...
For example, the "UpsertUser" mutation has 2 optional variables ("displayName" and "email")
and can be executed via the [DefaultConnector.upsertUser]
property as follows:

```kotlin
val connector = DefaultConnector.instance
val mutationResult = connector.upsertUser.execute {
  displayName = "garply"
  email = "qux"
}
