...
- **Data Type**: APPROX_LOCATION
  *Description: User or device physical location to an area greater than or equal to 3 square kilometers, such as the city a user is in, or location provided by Android’s ACCESS_COARSE_LOCATION permission.*
...
- **Data Type**: DEVICE_ID
  *Description: Identifiers that relate to an individual device, browser or app. For example, an IMEI number, MAC address, Widevine Device ID, Firebase installation ID, or advertising identifier.*
  - `dist/assets/index-BjcG2aKc.js (Pattern: instanceId)`
...
- **Data Type**: FITNESS
  *Description: Information about a user's fitness, such as exercise or other physical activity.*
  - `tmp/compose-samples/JetLagged/app/src/main/java/com/example/jetlagged/JetLaggedScreen.kt (Pattern: HeartRate)`
- `tmp/compose-samples/JetLagged/app/src/main/java/com/example/jetlagged/data/FakeHeartRateData.kt (Pattern: HeartRate)`
- `tmp/compose-samples/JetLagged/app/src/main/java/com/example/jetlagged/data/JetLaggedHomeScreenState.kt (Pattern: HeartRate)`
...
Save final JSON output to `/home/shaolin/lyria/.scratch/play_policy_insights_c1b5951b-0984-47af-99a6-f147b63efb14/worker_data_safety_part_10.json`.
