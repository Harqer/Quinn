...
3. **Naming**: The package name (`com.example.jetcaster.tv`) and app label
   (`Mave`) often describe the app's purpose explicitly.
...
- **Evidence**:
  ...
  - **Precise Location usage**:
    - `dist/assets/index-BjcG2aKc.js (Pattern: latitude)`
- `shared/src/androidMain/kotlin/com/musically/studio/location/LocationManager.kt (Pattern: FusedLocationProviderClient)`
- `shared/src/androidMain/kotlin/com/musically/studio/ui/MainViewModel+Concerts.kt (Pattern: latitude)`
  
  
  - **Approximate Location usage**:
    - `shared/src/androidMain/kotlin/com/musically/studio/location/LocationManager.kt (Pattern: LocationManager)`
- `shared/src/androidMain/kotlin/com/musically/studio/ui/MainViewModel+Concerts.kt (Pattern: LocationManager)`
- `shared/src/androidMain/kotlin/com/musically/studio/ui/MainViewModel.kt (Pattern: LocationManager)`
...
- **Common Evaluation Matrix**:
  ...
  | **Foreground** | Any | App purpose (`Mave`) does not imply location, yet foreground tracking is used, and prominent disclosure alert code is missing. | `IMPORTANT` | Implement a **Prominent In-App Disclosure** dialog explaining what location data is collected *before* requesting foreground permission. |
...
- **Domain-Specific Heuristics (Strictly Bounded)**:
  ...
  1. **The Foreground Sufficiency Test**: Analyze background threads,
     ...
     `IMPORTANT` violation.
  2. **The Reasonable Expectation Test**: If the app label (`Mave`) or
...
- **Evidence**:
  - `app/src/main/java/com/musically/studio/appfunctions/MaveFunctions.kt (Pattern: startForeground)`
- `shared/src/androidMain/kotlin/com/musically/studio/WearableStreamingService.kt (Pattern: FOREGROUND_SERVICE)`
- `shared/src/androidMain/kotlin/com/musically/studio/ui/MainViewModel+Wearable.kt (Pattern: startForeground)`
- `shared/src/androidMain/kotlin/com/musically/studio/ui/MainViewModel.kt (Pattern: startForeground)`
  **Relevant Permissions Requested**:
...
- `components/organisms/chat-interface.styles.ts (Pattern: record)`
- `components/organisms/chat-interface.ts (Pattern: record)`
  **Relevant Permissions Requested**:
...
Save final JSON output to `/home/shaolin/lyria/.scratch/play_policy_insights_c1b5951b-0984-47af-99a6-f147b63efb14/worker_{{GOAL_NAME}}.json`.
