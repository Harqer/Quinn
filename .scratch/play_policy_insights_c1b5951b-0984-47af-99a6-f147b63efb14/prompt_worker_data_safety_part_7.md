...
- **Data Type**: OTHER_APP_PERFORMANCE
  *Description: Any other app performance data excluding crash log data (e.g., stack traces) and diagnostics (e.g., battery life, loading time, latency, or framerate).*
  - `components/organisms/android-library.styles.ts (Pattern: telemetry)`
- `components/organisms/android-library.ts (Pattern: telemetry)`
- `components/organisms/security-hub.ts (Pattern: telemetry)`
- **Data Type**: NAME
  *Description: How a user refers to themselves, such as their first or last name, or nickname.*
  - `components/organisms/android-library.ts (Pattern: display_name)`
- `tmp/compose-samples/Jetchat/app/src/main/java/com/example/compose/jetchat/profile/Profile.kt (Pattern: display_name)`
- `tmp/compose-samples/Jetchat/app/src/main/res/values/strings.xml (Pattern: display_name)`
...
Save final JSON output to `/home/shaolin/lyria/.scratch/play_policy_insights_c1b5951b-0984-47af-99a6-f147b63efb14/worker_data_safety_part_7.json`.
