...
private fun NavigateToDiscover(onNavigationRequested: () -> Unit, modifier: Modifier = Modifier) {
    ...
    Box(modifier = modifier, contentAlignment = Alignment.Center) {
        Column {
            Text(
                text = stringResource(R.string.display_no_subscribed_podcast),
                style = MaterialTheme.typography.displayMedium,
            )
            Text(text = stringResource(R.string.message_no_subscribed_podcast))
            Button(
                ...
                Text(text = stringResource(R.string.label_navigate_to_discover))
            }
        }
    }
}
