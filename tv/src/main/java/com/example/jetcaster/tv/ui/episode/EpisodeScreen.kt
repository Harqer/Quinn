...
private fun EpisodeInfo(playerEpisode: PlayerEpisode, playEpisode: () -> Unit, addPlayList: () -> Unit, modifier: Modifier = Modifier) {
    val duration = playerEpisode.episodeInfo.duration

    Column(modifier) {
        Text(text = playerEpisode.podcastInfo.author, style = MaterialTheme.typography.bodySmall)
        Text(text = playerEpisode.episodeInfo.title, style = MaterialTheme.typography.headlineLarge)
        if (duration != null) {
            EpisodeDataAndDuration(offsetDateTime = playerEpisode.episodeInfo.published, duration = duration)
        }
        Spacer(modifier = Modifier.height(JetcasterAppDefaults.gap.paragraph))
        Text(
            text = playerEpisode.episodeInfo.summary,
            softWrap = true,
            maxLines = 5,
