...
async function checkFreeQuota(uid: string) {
  // We check custom claims for the subscription tier (FREE, PREMIUM, FAMILY)
  const userRecord = await getAuth().getUser(uid);
  const tier = userRecord.customClaims?.subscriptionTier || 'FREE';

  if (tier === 'PREMIUM' || tier === 'FAMILY') {
    // Unlimited generations for paying tiers
    return;
  }

  // Enforce 1 free generation for FREE tier
  ...
  // Increment quota for the free user
  await quotaRef.set({ generations: FieldValue.increment(1) }, { merge: true });
}
