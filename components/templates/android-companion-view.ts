 ...
 * @SemanticPurpose: UI Component for android-companion-view.ts
 */
...
import '../atoms/android-welcome';
import './android-onboarding-flow';
import '../molecules/android-search-home';
import '../molecules/android-search-results';
import './android-album-details';
import '../molecules/android-community';
import '../molecules/android-go-live';
import '../organisms/android-library';
import '../molecules/android-options-menu';
...
export class AndroidCompanionView extends LitElement {
  ...
  private toggleAndroidCamera() { this.dispatchEvent(new CustomEvent('toggle-camera', { bubbles: true, composed: true })); }
  private togglePlayback() { this.dispatchEvent(new CustomEvent('toggle-play', { bubbles: true, composed: true })); }
  private goBackAndroidStep() { this.dispatchEvent(new CustomEvent('go-back', { bubbles: true, composed: true })); }
  ...
  private handlePlaySong(e: any) {
    const detail = e.detail;
    if (Array.isArray(detail)) {
      const [title, artist, cover] = detail;
      if (title) this.androidActiveSongTitle = title;
      if (artist) this.androidActiveSongArtist = artist;
      if (cover) this.androidActiveSongCover = cover;
    } else if (typeof detail === 'string') {
      this.androidActiveSongTitle = detail;
    }
    this.isAndroidPlaying = true;
    this.playbackState = "playing";
  }

  private handleAddToQueue(e: any) {
    this.dispatchEvent(new CustomEvent('add-to-queue', { detail: e.detail, bubbles: true, composed: true }));
  }

  private handleAddToPlaylist(e: any) {
    this.dispatchEvent(new CustomEvent('add-to-playlist', { detail: e.detail, bubbles: true, composed: true }));
  }

  private handleGoToRadio(e: any) {
    this.dispatchEvent(new CustomEvent('go-to-radio', { detail: e.detail, bubbles: true, composed: true }));
  }

  private handleLikeAllSongs() {
    this.dispatchEvent(new CustomEvent('like-all-songs', { bubbles: true, composed: true }));
  }

  private handleViewArtist(e: any) {
    this.dispatchEvent(new CustomEvent('view-artist', { detail: e.detail, bubbles: true, composed: true }));
  }
      ...
      <div class="android-simulation-container">
        <div class="android-phone-frame">
          ...
          <div class="android-phone-screen">
            ...
            ${this.androidFlowStep === "search_home" ? html`<android-search-home @step-change=${(e: any) => this.androidFlowStep = e.detail} @play-song=${this.handlePlaySong}></android-search-home>` : ""}
            ${this.androidFlowStep === "search_results" ? html`<android-search-results @step-change=${(e: any) => this.androidFlowStep = e.detail} @play-song=${this.handlePlaySong}></android-search-results>` : ""}
            ${this.androidFlowStep === "album_details" ? html`<android-album-details @step-change=${(e: any) => this.androidFlowStep = e.detail}></android-album-details>` : ""}
            ${this.androidFlowStep === "community" ? html`<android-community @step-change=${(e: any) => this.androidFlowStep = e.detail} @play-song=${this.handlePlaySong}></android-community>` : ""}
            ${this.androidFlowStep === "go_live" ? html`<android-go-live @step-change=${(e: any) => this.androidFlowStep = e.detail}></android-go-live>` : ""}
            ${this.androidFlowStep === "library" ? html`<android-library @step-change=${(e: any) => this.androidFlowStep = e.detail}></android-library>` : ""}
          </div>
          ...
          ${this.isAndroidOptionsMenuOpen ? html`<android-options-menu
            @add-to-queue=${this.handleAddToQueue}
            @add-to-playlist=${this.handleAddToPlaylist}
            @go-to-radio=${this.handleGoToRadio}
            @like-all-songs=${this.handleLikeAllSongs}
            @view-artist=${this.handleViewArtist}
