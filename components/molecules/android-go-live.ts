 ...
 * @SemanticPurpose: UI Component for android-go-live.ts
 */
    ...
    return html`
      <div class="android-flow-go-live animate-fade-in">
        <div class="community-header-row" style="margin-bottom: 12px;">
          ...
          <button class="go-live-pulsing-btn" style="background: #ef4444; color: white;" @click=${() => { this.dispatchEvent(new CustomEvent("step-change", { detail: "community", bubbles: true, composed: true })); this.dispatchEvent(new CustomEvent('stop-broadcast', { bubbles: true, composed: true })); if (this.isHapticsEnabled && navigator.vibrate) navigator.vibrate(15); }}>
            <span>END</span>
          </button>
        </div>
