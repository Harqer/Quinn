 ...
 * @SemanticPurpose: UI Component for android-community.ts
 */
      ...
      <div class="android-flow-community animate-fade-in">
        ...
        <div class="community-chat-container">
          ...
          <div class="chat-bubbles-scroll">
            ${!this.communityComments || this.communityComments.length === 0 ? html`
              <div class="chat-bubble-card skeleton-loader" style="height: 80px; margin-bottom: 12px; border-radius: 8px;"></div>
              <div class="chat-bubble-card skeleton-loader" style="height: 80px; margin-bottom: 12px; border-radius: 8px;"></div>
              <div class="chat-bubble-card skeleton-loader" style="height: 80px; margin-bottom: 12px; border-radius: 8px;"></div>
            ` : this.communityComments.map(comment => html`
              <div class="chat-bubble-card" @click=${() => {
                this.dispatchEvent(new CustomEvent("play-song", { detail: [comment.user + "'s Choice", "Live Request", comment.coverUrl || "", comment.vibe || ""], bubbles: true, composed: true }));
                this.dispatchEvent(new CustomEvent('play-request', { detail: { user: comment.user }, bubbles: true, composed: true }));
              }}>
