...
rm -f tmp/r8analysis/keepruleradius.json tmp/r8analysis/*.pb

./gradlew assembleRelease -Dcom.android.tools.r8.dumpkeepradiustodirectory=$PWD/tmp/r8analysis

cat << 'EOF' > tmp/r8analysis/keep_radius_pb2.py
...
cat << 'EOF' > tmp/r8analysis/convert.py
import sys
import os
import glob
...
cat << 'EOF' > tmp/r8analysis/analyze.py
import json, sys
...
cat << 'EOF' > tmp/r8analysis/report.py
import json, sys
...
python3 tmp/r8analysis/convert.py
python3 tmp/r8analysis/analyze.py
python3 tmp/r8analysis/report.py
