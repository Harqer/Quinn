...
with open('./components/organisms/top-nav.ts', 'r') as f:
    classes, ids = extract_classes_and_ids(f.read())
with open('./components/organisms/top-nav.styles.ts', 'w') as f:
    f.write(f"import {{ css }} from 'lit';\n\nexport const topNavStyles = css`\n{extract_css(classes, ids)}\n`;\n")

with open('./components/organisms/bottom-nav.ts', 'r') as f:
    classes, ids = extract_classes_and_ids(f.read())
with open('./components/organisms/bottom-nav.styles.ts', 'w') as f:
    f.write(f"import {{ css }} from 'lit';\n\nexport const bottomNavStyles = css`\n{extract_css(classes, ids)}\n`;\n")

print("Extracted CSS for nav components")
